...
# @Last Modified time: 2017-05-14 18:33:43
  ...
  for file in os.listdir(filepath):
    if file.endswith('.txt'):
      ...
      with open(os.path.join(filepath, file), 'r', encoding='utf-8') as json_file:
        pages = loads(json_file.read())
        doc = {}
        doc[file] = list()
        for page_number, page_content in pages.items():
          words = standardize_words([word for word in page_content])
          doc[file].extend(words)
        with open('output_s_jsons/{}'.format(file), 'w') as s_word_open:
          s_word_open.write(dumps(doc))
        index = 0
        for word in words:
          tokens.setdefault(word, file)
          corpus[word].append((file, index))
          index += 1
        if (file in corpus.keys()):
