 ...
 * Original:David Le (Student ID: 008-144-669)
 * January 18, 2016
 * Modified by Wei-Chugn Huang
 * Last modifed: 5/15/2017
 */
...
// Modified by Wei-Chung Huang:
// Added a flag to output an intermidate bitmap of processing"
#undef DUMP_BITMAP
...
struct Simplex {
    ...
    int nrules;                   // # of rules: 1, 2, 3 and etc
    vector<int> ones;               // [row#] -- inverted list of 1's
    vector<pair<int, vector<int> > > links; // [simplex, links] -- connected vertices on graph with AND'ed ones
};

vector<vector<bool> > data;         // [col][row#] -- input data matrix
vector<vector<int> > lookup;        // [col][row#] -- inverted list of 1's
vector<pair<int, int> > qcols;    // [simplex, count] -- sorted list of qualified counts
map<int, int> results;            // [simplex, #items]
FILE* fpResult;
...
void reduceSpace();
bool compareCounts(pair<int, int>& elem1, pair<int, int>& elem2);
void buildSimplex(Simplex& simplex, string& gname, int nrules, vector<int>& ones, vector<pair<int, int> >& cols, int start);
...
int main (int argc, char* argv[])
{
    ...
    if (argc < 7) { 
        cout << "Usage: SimplicialComplex <#rules> <threshold> <#columns> <#records> <data_file> <output_file>[-brief]" << endl; 
        return 0; 
    }
    ...
    if (!brief) {
        fpResult = fopen(outputFile.c_str(), "wt");
    ...
    cout << "ThresholdLimit: " << thresholdLimit << endl;
    ...
    cout << "qcol size: " << qcols.size() << endl;
    // return 1;
    ...
    map<int, int>::iterator shape;
    for (int index = (int)qcols.size() - 1; index >= 0; index--) {
        int col = qcols[index].first;
        string gname = to_string(col);
        Simplex simplex;
        // cout << "A: index: col: " << index << " " << col << endl;
...
bool readFile(string strFile, int numCols, int numRows)
{
    ...
    int length, col;
    int max_lookup_size = -1;
    for (col = 0; col < numCols; col++) {
        lookup[col].reserve(numRows);
        if (lookup[col].size() > max_lookup_size) max_lookup_size = lookup[col].size();
    }
    cout << "Lookup max size: " << max_lookup_size << endl;
    
    int max_length = -1;

    int max_col_length = -1;
    unsigned int min_col_length = 2^32;

    for (int row = 0; row < numRows; row++) {
        fscanf(fp, "%d", &length);

        if (length > max_col_length) max_col_length = length;
        if (length < min_col_length) min_col_length = length;
    ...
    cout << "max col length: " << max_col_length << " min: " << min_col_length << endl;
...
void reduceSpace()
{
    ...
    for (int col = 0; col < (int)lookup.size(); col++) {
        ...
        if (count >= thresholdLimit) {
            qcols.push_back(pair<int, int>(col, count));
        }
    }
    qcols.shrink_to_fit();
...
void buildSimplex(Simplex& simplex, string& gname, int nrules, vector<int>& ones, vector<pair<int, int> >& cols, int start)
{
    ...
    if (nlinks > 0) {
        ...
        for (int index = start; index < cols.size(); index++) {
            int col = cols[index].first;
            pair<int, vector<int> > link;
...
void connectSimplex(Simplex& simplex)
{
    ...
    if (simplex.nrules < numRules) {
        for (int i = 0; i < simplex.links.size(); i++) {
            pair<int, vector<int> >& link = simplex.links[i];
            ...
            for (int j = i + 1; j < simplex.links.size(); j++) {
                pair<int, vector<int> >& sublink = simplex.links[j];
