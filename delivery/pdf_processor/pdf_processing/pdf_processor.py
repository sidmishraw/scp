...
# @Last Modified time: 2017-04-17 22:50:39
...
# logger settings
logger = getLogger('pdf_processor')
handler = FileHandler('pdfprocessor.log')
...
# These are keys defined in pdfminer module for building the dicts used in
# the module.
PAGES = 'Pages'
...
# Globals
# The global dict that holds the mapping of the page to its corressponding
# contents.
...
def __extract_pages__(resolved_obj, pages):
  ...
  if isinstance(resolved_obj[TYPE], PSLiteral) and resolved_obj[
          TYPE].name == PAGE:
    pages.append(resolved_obj)
    return
  elif isinstance(resolved_obj[TYPE], PSLiteral) and resolved_obj[TYPE].name == PAGES:
    for objref in resolved_obj[KIDS]:
      __extract_pages__(objref.resolve(), pages)
    return


# This function will extract the pages from the PDF document using the
# PDF's catalog
...
def get_pdf_contents(pdf_file_name):
  ...
  with open(pdf_file_name, 'rb') as ip_file:
    ...
    logger.error('HARMLESS: extracting pages of {}'.format(pdf_file_name))

    pages = extract_pages(doc)

    logger.error(
        'HARMLESS: finished extracting pages of {}'.format(pdf_file_name))

    logger.error(
        'HARMLESS: started extracting page contents of {}'.format(pdf_file_name))
    ...
    for page_nbr, page in enumerate(pages):
      try:
        ...
        logger.error(
            'ERROR: The pdfdocument {} maybe a scanned image.'.format(pdf_file_name))

    logger.error(
        'HARMLESS: finished extracting page contents of {}'.format(pdf_file_name))

  return dict(__pdf_contents_dict__)


# Writes the Page:Contents mapping to the JSON file
# Writing to a JSON file can help if we need to use someother
# platform/stack for this program.
...
def __extract_content__(matching_pattern):
  ...
  matching_patterns = findall(
      compile(text_digit_regex_pattern),
      matching_pattern)
...
def __get_content__(content):
  ...
  while index < len(matching_patterns) and seeker < len(matching_patterns):
    ...
    if matching_patterns[seeker][5] == '':
      contents[contents_key].append(
          __extract_content__(
              matching_patterns[seeker][4]))
    else:
      contents[contents_key].append(
          __extract_content__(
              matching_patterns[seeker][5]))
...
def __cleanse_words__(word_group, cleansed_word_container):
  ...
  for word in word_group:
    if len(
            cleansed_word_container) != 0 and cleansed_word_container[-1].endswith('-'):
      cleansed_word_container[-1] = '{}{}'.format(
          cleansed_word_container[-1][:-1], word)
    else:
...
def __cleanse_page_contents__(page_contents):
  ...
  cleansed_page_contents = list(
      map(
          __leading_trailing_nonword_remover__,
          cleansed_page_contents))
    ...
    for content in page_contents.values():
      ...
      for matching_pattern in matching_patterns:
        if matching_pattern[5] == '':
          pdf_page_words_dict[page].extend(
              __extract_content__(matching_pattern[4]))
        else:
          pdf_page_words_dict[page].extend(
              __extract_content__(matching_pattern[5]))
